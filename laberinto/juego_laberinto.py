...
# --- CONSTANTES ---
SAVE_FILE = "savegame.json" # Nombre del archivo de guardado
...
comet_count = 0
current_score = 0 # Variable global de puntuación
...
def load_game():
    ...
    try:
        with open(SAVE_FILE, "r") as f:
            ...
            # Cargar la puntuación. Si no existe en el save, default a 0.
            if 'current_score' not in state:
...
def run_level(level, load_data=None):
    ...
    # Usar las variables globales de estado
    global current_lives, comet_count, current_score
    ...
    if load_data is None:
        ...
        if level == 1:
            # main() se encarga de resetear vidas/cometas/puntos
            pass
    ...
    def render_hud():
        global current_score # Asegurarse de que usa la global
        life_display = (HEART_STR * current_lives) + (EMPTY_HEART_STR * (MAX_LIVES - current_lives))
        
        # Añadida la puntuación al HUD
        hud_line = f"VIDA: {life_display} | NIVEL: {level}/3 | ☄️: {comet_count} | PUNTOS: {current_score}"
        controls_line = "(P: Visión, E: Romper, B: IA, Q: Guardar/Salir)"
        
        sys.stdout.write(f"\033[1;1H" + " " * (CELL_W * CHAR_SIZE + 15)) # Expandir limpieza
        sys.stdout.write(f"\033[2;1H" + " " * (CELL_W * CHAR_SIZE + 15))
    ...
    def handle_trap_collision(new_pos):
        global current_lives, current_score
        if new_pos in traps_pos:
            current_lives -= 1
            current_score -= 100 # Restar puntos por trampa
    ...
    def handle_collectible_collision(new_pos):
        global comet_count, current_score
        if new_pos in comets_pos:
            comet_count += 1
            current_score += 50 # Sumar puntos por cometa
    ...
    def handle_heart_collision(new_pos):
        ...
        if new_pos in hearts_pos:
            if current_lives < MAX_LIVES:
                current_lives += 1
                current_score += 300 # Sumar puntos por corazón
    ...
    render_hud() # render_hud ya usa la global current_score
    ...
    try:
        while True:
            ...
            if ch:
                ...
                # 'Q' (Guardar y Salir)
                if c == 'q':
                    # Añadir puntuación al estado de guardado
                    game_state = {
                        'level': level,
                        'current_lives': current_lives,
                        'comet_count': comet_count,
                        'current_score': current_score, # Guardar puntuación
                        'player': player,
                        'exitp': exitp,
                        'traps_pos': traps_pos,
                    ...
                    sys.stdout.write(f"\033[{3+CHAR_SIZE+3};1HPartida guardada en {SAVE_FILE} (Puntos: {current_score})\n")
                    sys.stdout.flush()
                    return "QUIT_SAVE"
                    ...
                    if comet_count > 0:
                        ...
                        for di, dj in [(-1,0), (1,0), (0,-1), (0,1)]:
                            ...
                            if 0 <= ti < CHAR_SIZE and 0 <= tj < CHAR_SIZE and mat[ti][tj] == SNOW_WALL:
                                ...
                                comet_count -= 1
                                current_score += 150 # (NUEVO) Puntos por romper muro
                                render_hud()
                                write_cell_with_visibility(ti, tj, PATH_STR, top=3, left=1)
                                sys.stdout.write(f"\033[{3+CHAR_SIZE+2};1HPared de nieve rota. 💥 (+150 pts)" + " "*20 + "\n"); sys.stdout.flush()
                                broken = True
                                break
                        if not broken:
                ...
                if not ai_active and c in ('w','a','s','d'):
                    ...
                    if can_move(mat, new):
                        ...
                        trail_set.add(prev)
                        update_visibility(player[0], player[1])
                        current_score -= 5 # (NUEVO) Restar puntos por paso
                        ...
                        render_hud() # Actualizar HUD por el paso
                        ...
                        if player == exitp:
                            sys.stdout.write(f"\033[{3+CHAR_SIZE+1};1H¡Nivel {level} completado! Puntuación actual: {current_score}\n"); sys.stdout.flush()
                            return "WIN"
            ...
            if ai_active:
                ...
                if now - last_ai >= 0.1:
                    ...
                    if nextp is None:
                        ...
                        trail_set.add(prev)
                        update_visibility(player[0], player[1])
                        current_score -= 5 # (NUEVO) Restar puntos por paso de IA
                        ...
                        if player == exitp:
                            sys.stdout.write(f"\033[{3+CHAR_SIZE+1};1HLa IA encontró la salida. Nivel {level} completado. Puntuación: {current_score}\n"); sys.stdout.flush()
                            return "WIN"
...
def main():
    global current_lives, comet_count, current_score # (MODIFICADO)
    MAX_LEVELS = 3
    load_data = None
    game_running = True
    ...
    while game_running:
        ...
        if load_data:
            ...
            # Cargar estado global desde el archivo
            current_lives = load_data['current_lives']
            comet_count = load_data['comet_count']
            current_score = load_data['current_score'] 
        else:
            level = 1
            # Resetear estado global para una nueva partida
            current_lives = MAX_LIVES
            comet_count = 0
            current_score = 1000 # Puntuación inicial
        ...
        while level <= MAX_LEVELS:
            ...
            if level_result == "WIN":
                if level == MAX_LEVELS:
                    ...
                    sys.stdout.write(f"\033[{3+CHAR_SIZE+4};1H🎉 Puntuación Total Final: {current_score} 🎉\n"); sys.stdout.flush()
                    game_running = False
                    break
                else:
                    level += 1
