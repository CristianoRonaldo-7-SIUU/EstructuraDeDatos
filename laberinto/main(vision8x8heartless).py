...
def run_level(level):
    ...
    def update_visibility(r, c):
        # La celda del jugador y sus 8 adyacentes son visibles
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                vr, vc = r + dr, c + dc
                if 0 <= vr < CHAR_SIZE and 0 <= vc < CHAR_SIZE:
                    explored_map[vr][vc] = True
    
    def render_level_with_visibility(top=3, left=1):
    ...
    render_hud()
    render_level_with_visibility(top=3, left=1)
            ...
            if ch:
                ...
                if not ai_active and c in ('w','a','s','d'):
                    ...
                    if can_move(mat, new):
                        ...
                        update_visibility(player[0], player[1]) # Revelar
                        ...
                        # Re-dibujar
                        if player_view_mode:
                            render_level_with_visibility(top=3, left=1) # Revelar
                        else:
